...
class MockLoadSheddingProvider(LoadSheddingProvider):
    """Deterministic Stage 4 day: outages at 14:00-16:30 and 20:00-22:30."""
    ...
    def get_schedule(self, area_id: str) -> AreaSchedule:
        return AreaSchedule(
            ...
            current_stage=4,
            windows=[
                OutageWindow(stage=4, start=_today_at(14), end=_today_at(16, 30)),
                OutageWindow(stage=4, start=_today_at(20), end=_today_at(22, 30)),
                OutageWindow(
                    stage=4,
                    start=_today_at(6) + timedelta(days=1),
                    end=_today_at(8, 30) + timedelta(days=1),
                ),
            ],
        )
...
class MockCalendarService(CalendarService):
    """A demo workday: one meeting collides with the 14:00 outage."""

    def __init__(self) -> None:
        self._meetings: dict[str, Meeting] = {
            m.id: m
            for m in [
                Meeting(id="m1", title="Standup", start=_today_at(9), end=_today_at(9, 30)),
                Meeting(
                    id="m2",
                    title="Client demo (Zoom)",
                    start=_today_at(14, 30),
                    end=_today_at(15, 30),
                ),
                Meeting(id="m3", title="1:1 with manager", start=_today_at(17), end=_today_at(17, 30)),
            ]
        }
